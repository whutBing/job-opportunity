...
const createCompiler = rawOptions => {
	...
	// 1.通过new创建一个Compiler的实例
	const compiler = new Compiler(options.context, options);
	new NodeEnvironmentPlugin({
		infrastructureLogging: options.infrastructureLogging
	}).apply(compiler);
	// 2.注册我们传入的所有的webpack插件plugins
	...
	// 3.调用钩子environment/afterEnvironment函数
	compiler.hooks.environment.call();
	compiler.hooks.afterEnvironment.call();
	// 4.使用process函数处理其他的选项, 比如entry/output/devtool等
...
const webpack = /** @type {WebpackFunctionSingle & WebpackFunctionMulti} */ (
	...
	(options, callback) => {
		...
		// 如果调用webpack函数时有传入callback, 那么调用if, 否则调用else
		// 但是最终目的都是为了创建compiler对象, 是webpack的核心对象之一
		// 另一个核心对象叫做 compilation
		if (callback) {
			try {
